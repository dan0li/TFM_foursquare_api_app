    ...
    if location:
        if st.button("📡 Llamar a la API"):
            ...
            if len(data) < limit or not data:
                ...
                if lat and lon and 'radius' in parametros:
                    radius_km = parametros['radius'] / 1000
                    df_dataset = filtrar_pois_fast(dataset_path, location=(lat, lon, radius_km), query=query)
